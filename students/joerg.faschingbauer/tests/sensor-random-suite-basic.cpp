...
TEST(sensor_random_suite, basic2)
{
    RandomSensor rs(-273.15, 0);         // <--- give measurements between lo and hi

    double t = rs.get_temperature();
    ASSERT_GE(t, -273.15);                  // <--- greater equal to lo
    ASSERT_LE(t, 0);                  // <--- less equal to hi
}
